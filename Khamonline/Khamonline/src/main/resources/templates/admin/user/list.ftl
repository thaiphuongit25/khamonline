                ...
                <div class="box-body">
                    <form id="qForm" class="form-inline search-row">
                        ...
                        <div class="form-group">
                            <input type="text" name="name" class="form-control" value="${name}" placeholder="Vui lòng nhập các từ khóa">
                        </div>
                            ...
                            <tbody>
                                ...
                                ${role.name}
                                    ...
                                    <td class="text-center">
                                        <#if row.id != 1>
                                            <#if row.status == 0>
                                                <a href="javascript:void(0);" class="btn btn-xs btn-default" data-id="${row.id}" data-action="close">Tắt</a>
                                            <#else>
                                                <a href="javascript:void(0);" class="btn btn-xs btn-success" data-id="${row.id}" data-action="open">Kích hoạt</a>
                                            </#if>
                                            <a href="${base}/admin/user/pwd?id=${row.id}" class="btn btn-xs btn-success">Đổi mật khẩu</a>
...
$(function() {
    ...
    $('#dataGrid a[data-action="close"]').bind('click', function(){
		var that = $(this);
		layer.confirm('Sau khi tài khoản bị vô hiệu hóa，Sẽ không thể đăng nhập vào hệ thống. Bạn có chắc chắn muốn tắt tính năng này không?', {
            btn: ['Yes','No'],
            shade: false
        }, function(){
    ...
    $('#dataGrid a[data-action="open"]').bind('click', function(){
		var that = $(this);
		layer.confirm('Sau khi tài khoản được kích hoạt, bạn sẽ truy cập được vào các chức năng được phân quyền trong hệ thống, bạn có chắc chắn muốn kích hoạt không?', {
            btn: ['Yes','No'],
            shade: false
        }, function(){
