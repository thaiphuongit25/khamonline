...
public class UserTag extends SecureTag {
    ...
    public void render(Environment env, Map params, TemplateDirectiveBody body) throws IOException, TemplateException {
        if (getSubject() != null && getSubject().getPrincipal() != null) {
            log.debug("Đối tượng đã biết danh tính. Nội dung thẻ sẽ được đánh giá.");
            renderBody(env, body);
        } else {
            log.debug("Đối tượng không tồn tại hoặc có danh tính đã biết. Nội dung thẻ sẽ không được đánh giá.");
        }
    }
}